...
public class MethodGeneratorUtils {
    ...
    public static String getCondName(PluginConfiguration extentModelPlugin, String  poName) {
        //设置参数类型
        String condSuffix = extentModelPlugin.getProperty("condSuffix");
        String condName = poName + MethodUtils.toUpperCase(condSuffix);
        if (!StringUtility.stringHasValue(condName)) {
            condName = CommonConstant.COND_SUFFIX;
        }
        return condName;
    }
